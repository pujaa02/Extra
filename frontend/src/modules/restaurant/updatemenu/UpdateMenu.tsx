      ...
      .then((res) => {
        if (res.data.message === 'success') {
          ...
          Object.entries(result_data).forEach(([key, value]) =>
            setValue(key as keyof MenuAttributes, value)
          );
        }
      })
      .catch((error) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">Update Menu</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(handlemenu)} className="space-y-6">
          <div className="flex">
            <div className="form-group">
              ...
              <label
                htmlFor="image"
                className="text-center bg-red-600 w-56 p-2 block text-xl font-medium text-slate-200 font-bold"
              >
                Upload Image
              </label>
              <input
