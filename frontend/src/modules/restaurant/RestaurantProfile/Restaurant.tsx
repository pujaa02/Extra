...
import { Rest, RestaurantAttributes } from '../../../Types/restaurant.types';
...
const Restaurant: React.FC = () => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<RestaurantAttributes>({ defaultValues: restaurantdata });

  const handlerestaurant: SubmitHandler<RestaurantAttributes> = async (
    data: RestaurantAttributes
  ) => {
    ...
    try {
      await instance({
        ...
        if (
          ...
          res.data.message === 'restaurant updated Successfully'
        ) {
      ...
      await instance({
        ...
        if (res.data.message === 'success') {
          ...
          Object.entries(result_data).forEach(([key, value]) =>
            setValue(key as keyof Rest, value)
          );
        }
      });
    } catch (error) {
